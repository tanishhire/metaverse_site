...
const ExploreCard = ({ id, imgUrl, title, index, active, handleClick }) => (
  <motion.div
    ...
    transition-[flex] duration-[0.75] ease-out-flex cursor-pointer`}
  ...
  >
    ...
    {active !== id ? (
      ...
      <div className="absolute bottom-0 p-8 justify-start w-full flex-col bg-[rgba(0,0,0,0.5)] rounded-b-[24px]">
        ...
        <p className="text-white uppercase font-normal text-[16px] leading-[20px] ">Enter the Metaverse</p>
        <h2 className="mt-[24px] sm:text-[32px] text-[24px] font-semibold text-white">{title}</h2>
      </div>
    )}
